...
## Criando os dataframes 
    path= './Dados/EMA/Unidos_v2/states'
    errors = open('./nao-encontrado.txt','a')
    for (dirpath, dirnames, filenames) in walk(path):
       # print(dirpath)        
       # print(dirnames)
        if filenames !=[] and dirpath.find('Reg')==-1:
            filenames.sort()
            print(filenames)
            i = 0
            df_reg = pd.DataFrame(columns = ['Year', 'Month', 'Day', 'H(UTC)', 'PRECIPITACAO (mm)',
            ...
            while(i<len(filenames)):
                    ...
                    df_reg = df_reg.append(data)
                    print(filenames[i])
                    i+=1        
            print(dirpath)
            df_reg.to_csv(str(path + '/Reg/'+dirpath.split('/')[5]+'.csv'))
    errors.close()